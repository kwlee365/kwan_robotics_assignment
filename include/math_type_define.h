...
namespace DyrosMath
{
	...
	const static Eigen::Matrix3d SetTrajectoryRotation(double time,
		double time_0,
		double time_f,
		const Eigen::Matrix3d &rotation_0,
		const Eigen::Matrix3d &rotation_f)
	{
		...
		Eigen::Quaterniond q0(rotation_0);
		Eigen::Quaterniond q1(rotation_f);

		Eigen::Vector3d qs_ = QuinticSpline(time, time_0, time_f, 0, 0, 0, 1, 0, 0);

		Eigen::Quaterniond q_traj = q0.slerp(qs_(0), q1);

		Eigen::Matrix3d result;
		result = q_traj.toRotationMatrix();

		return result;
	}

	const static Eigen::Matrix3d SetTrajectoryRotation2(double time,
		double time_0,
		double time_f,
		const Eigen::Matrix3d &rotation_0,
		const Eigen::Matrix3d &rotation_f)
	{
		...
		Eigen::AngleAxisd aa(rotation_0.transpose() * rotation_f);
        axis = aa.axis();
        angle = aa.angle();

		double c_a = DyrosMath::cubic(time, time_0, time_f, 0.0, angle, 0.0, 0.0);
		Eigen::Matrix3d rmat;
		rmat = Eigen::AngleAxisd(c_a, axis);

		Eigen::Matrix3d result;
		result = rotation_0 * rmat;
	...
	static Eigen::Vector3d rot2Euler(Eigen::Matrix3d Rot)	// ZYX
	{
		double beta;
